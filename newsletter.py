"""
Al Dia - newsletter_generator.py
Datos de mercado obtenidos en Python durante el build (GitHub Actions).
No depende de JavaScript ni proxies en el navegador.
"""
...
# ── Market data — fetched server-side in Python ────────────────────────────
def get_market_data():
    """
    Fetch market data from free APIs during the GitHub Actions build.
    Returns a dict with values ready to embed in HTML.
    """
    markets = {
        ...
        "oil":     {"label":"Petróleo", "val":"—", "chg":"—", "up": None},
    }

    headers = {"User-Agent": "Mozilla/5.0 (compatible; newsletter-bot/1.0)"}

    # ── 1. Stocks & indices via Yahoo Finance query API ──────────────────
    try:
        syms = "%5EGSPC,%5EIXIC,GC%3DF,CL%3DF"
        url  = f"https://query1.finance.yahoo.com/v7/finance/quote?symbols={syms}&fields=regularMarketPrice,regularMarketChangePercent"
        ...
        for q in results:
            ...
            up    = chg >= 0
            chg_str = ("+" if up else "") + f"{chg:.2f}%"
            if sym == "^GSPC":
                markets["sp500"]  = {"label":"S&P 500", "val": f"{price:,.0f}", "chg": chg_str, "up": up}
            elif sym == "^IXIC":
                markets["nasdaq"] = {"label":"Nasdaq",  "val": f"{price:,.0f}", "chg": chg_str, "up": up}
            elif sym == "GC=F":
                markets["gold"]   = {"label":"Oro",     "val": f"${price:,.1f}", "chg": chg_str, "up": up}
            elif sym == "CL=F":
                markets["oil"]    = {"label":"Petróleo","val": f"${price:,.1f}", "chg": chg_str, "up": up}
        print(f"  Stocks OK: {[q.get('symbol') for q in results]}")
    except Exception as e:
        print(f"  Stocks error: {e}")

    # ── 2. EUR/USD via Frankfurter (European Central Bank data, free) ────
    try:
        req = Request("https://api.frankfurter.app/latest?from=EUR&to=USD", headers=headers)
        with urlopen(req, timeout=8) as r:
            data = json.loads(r.read().decode())
        rate = data["rates"]["USD"]
        # EUR/USD = how many USD per 1 EUR
        markets["eurusd"] = {"label":"EUR/USD", "val": f"{rate:.4f}", "chg": "", "up": None}
        print(f"  EUR/USD OK: {rate}")
    except Exception as e:
        print(f"  EUR/USD error: {e}")

    # ── 3. Bitcoin via CoinGecko (free, no key needed) ───────────────────
    try:
        ...
        markets["bitcoin"] = {
            "label": "Bitcoin",
            "val":  f"${price:,.0f}",
            "chg":  ("+" if up else "") + f"{chg:.2f}%",
            "up":   up
        }
        print(f"  Bitcoin OK: ${price:,.0f}")
    except Exception as e:
...
def market_cell(m):
    ...
    if m["up"] is True:
        ...
        arrow = ""
    chg_text = arrow + m["chg"] if m["chg"] else "—"
    return (
        '<div style="text-align:center;padding:0 4px;">'
        '<div style="font-size:8px;letter-spacing:.5px;text-transform:uppercase;'
        'color:rgba(255,255,255,.4);margin-bottom:2px;">' + m["label"] + '</div>'
        '<div style="font-size:12px;font-weight:600;color:' + val_color + ';">' + m["val"] + '</div>'
        '<div style="font-size:10px;color:' + chg_color + ';">' + chg_text + '</div>'
        '</div>'
    )

markets_html = "".join(market_cell(m) for m in markets.values())
print("Markets:", {k: v["val"] for k,v in markets.items()})
...
GREETINGS = [
    ...
    "Mitad de semana, mitad del mundo cubierto. Empecemos con lo mas importante de hoy 🌍",
    "Jueves — ya casi viernes. Mientras tanto, esto es lo que no te puedes perder hoy 📰",
    "Viernes! Antes de desconectarte, tomaté 3 minutos para saber que paso esta semana 🎉",
    "Fin de semana, pero el mundo no para. Aqui el resumen de lo importante este sabado 🌅",
    "Domingo de lectura. El mejor momento para ponerte al dia antes de que empiece la semana ☀️",
]
greeting = GREETINGS[now_et.weekday()]

# ── Historias del día ──────────────────────────────────────────────────────
HISTORIAS = [
    {"titulo":"La historia del Dia del Trabajo","emoji":"✊",
     "texto":"El 1 de mayo de 1886, miles de trabajadores en Chicago salieron a las calles exigiendo una jornada laboral de 8 horas. Lo que empezo como huelga termino en tragedia cuando una bomba estallo en la plaza Haymarket. Cuatro activistas fueron ejecutados. Decadas despues, el mundo adopto el 1 de mayo como Dia Internacional del Trabajo en su memoria.",
     "dato":"EE.UU. y Canada celebran su Labor Day en septiembre, justamente para diferenciarse de las raices socialistas del 1 de mayo."},
    {"titulo":"El dia que pisamos la Luna","emoji":"🌕",
     "texto":"El 20 de julio de 1969, Neil Armstrong bajo del modulo lunar Eagle y dijo una de las frases mas famosas de la historia. Cerca de 600 millones de personas lo vieron en vivo por television. La mision Apollo 11 fue el resultado de una carrera espacial de una decada entre EE.UU. y la Union Sovietica.",
     "dato":"Los astronautas dejaron en la Luna una placa que dice: Vinimos en paz de parte de toda la humanidad."},
    {"titulo":"La caida del Muro de Berlin","emoji":"🧱",
     "texto":"El 9 de noviembre de 1989, un portavoz del gobierno de Alemania Oriental anuncio por error que los ciudadanos podian cruzar la frontera de inmediato. Miles corrieron al muro. Los guardias, sin ordenes claras, abrieron los checkpoints. Esa noche la gente empezo a derribarlo con martillos. El simbolo de la Guerra Fria cayo sin una sola bala.",
     "dato":"El Muro de Berlin tenia 155 km de longitud y estuvo en pie 28 anos, 2 meses y 27 dias."},
    {"titulo":"El nacimiento de los Estados Unidos","emoji":"🦅",
     "texto":"El 4 de julio de 1776, el Congreso Continental adopto la Declaracion de Independencia, redactada principalmente por Thomas Jefferson. Las 13 colonias americanas se separaban oficialmente de Gran Bretana. Pocos saben que la declaracion fue aprobada el 2 de julio — el 4 fue cuando se termino de imprimir.",
     "dato":"John Adams y Thomas Jefferson murieron el mismo dia: el 4 de julio de 1826, exactamente 50 anos despues de la Declaracion."},
    {"titulo":"El dia que Mandela salio libre","emoji":"🕊️",
     "texto":"El 11 de febrero de 1990, despues de 27 anos en prision, Nelson Mandela camino libre por las puertas de la carcel Victor Verster en Sudafrica. El mundo entero lo esperaba. Cuatro anos despues se convirtio en el primer presidente negro de Sudafrica, elegido en las primeras elecciones democraticas del pais.",
     "dato":"Mandela paso 18 de sus 27 anos de prision en la isla Robben, en una celda de apenas 2x2 metros."},
    {"titulo":"El Titanic y la noche mas larga","emoji":"🚢",
     "texto":"En la madrugada del 15 de abril de 1912, el RMS Titanic se hundio en el Atlantico Norte. De 2,224 personas a bordo, solo 710 sobrevivieron. El barco era considerado insumergible, pero llevaba botes salvavidas para menos de la mitad de los pasajeros. La tragedia cambio para siempre las regulaciones maritimas internacionales.",
     "dato":"El barco Carpathia respondio al SOS y navego 93 km a toda velocidad para rescatar a los sobrevivientes."},
    {"titulo":"Yuri Gagarin y el primer viaje al espacio","emoji":"🚀",
     "texto":"El 12 de abril de 1961, Yuri Gagarin se convirtio en el primer ser humano en viajar al espacio. Su vuelo duro 108 minutos orbitando la Tierra. La hazana sovietica sacudio a EE.UU. y acelero el programa espacial que llevaria al hombre a la Luna ocho anos despues.",
     "dato":"Gagarin murio en 1968 en un accidente de entrenamiento, a los 34 anos, sin llegar a ver el alunizaje que su vuelo inspiro."},
    {"titulo":"El discurso que cambio America","emoji":"✊",
     "texto":"El 28 de agosto de 1963, Martin Luther King Jr. se paro frente a 250,000 personas en Washington D.C. y pronuncio su famoso discurso. Gran parte fue improvisado. Sus palabras sobre igualdad racial se convirtieron en uno de los momentos mas poderosos del movimiento por los derechos civiles en EE.UU.",
     "dato":"King tenia solo 34 anos cuando dio ese discurso. Cinco anos despues fue asesinado en Memphis, Tennessee."},
    {"titulo":"Einstein y la formula que cambio el mundo","emoji":"🧠",
     "texto":"El 14 de marzo de 1879 nacio Albert Einstein. De nino sus profesores lo creian lento. A los 26 anos, trabajando como empleado en una oficina de patentes, publico cuatro articulos que revolucionaron la fisica — incluyendo E=mc2, que demostro que la materia y la energia son la misma cosa.",
     "dato":"Einstein gano el Nobel en 1921, pero no por la relatividad — sino por su explicacion del efecto fotoelectrico."},
    {"titulo":"El origen del Ano Nuevo","emoji":"🎆",
     "texto":"Celebrar el inicio del ano es una tradicion de mas de 4,000 anos. Los primeros fueron los babilonios. Fueron los romanos quienes movieron la celebracion al 1 de enero, en honor a Jano, el dios de los comienzos — de ahi viene January en ingles.",
     "dato":"En Japon, el Ano Nuevo (Oshoogatsu) es la festividad mas importante del ano, con 3 dias de celebracion oficial."},
    {"titulo":"Cristobal Colon y el encuentro de dos mundos","emoji":"⛵",
     "texto":"El 12 de octubre de 1492, Rodrigo de Triana grito Tierra desde la carabela Pinta. Colon penso que habia llegado a Asia. En realidad, habia pisado el Caribe. Ese momento conecto dos mundos que no sabian que existian el uno al otro, cambiando la historia de la humanidad para siempre.",
     "dato":"Colon murio en 1506 creyendo que habia llegado a Asia — nunca supo que habia descubierto un continente nuevo."},
    {"titulo":"Hiroshima y el inicio de la era nuclear","emoji":"☁️",
     "texto":"El 6 de agosto de 1945, el bombardero Enola Gay lanzo sobre Hiroshima la primera bomba atomica usada en combate. En segundos murieron 80,000 personas. Japon se rindio el 15 de agosto, terminando la Segunda Guerra Mundial y comenzando la era nuclear.",
     "dato":"Un arbol ginkgo biloba plantado a 1 km del epicentro sobrevivio la explosion. Hoy sigue vivo y se lo llama el arbol superviviente."},
]
...
nd = (mundial + otros)[:3]
print("OK Int:" + str(len(ni)) + " Ecu:" + str(len(ne)) + " Dep:" + str(len(nd)) + " Mundial:" + str(len(mundial)))
...
def top_card(a, grad, tag):
    return (
        ...
        'padding:2px 9px;border-radius:20px;margin-bottom:7px;">' + tag + '</span><br>'
        '<a href="' + a['link'] + '" target="_blank" style="font-family:Georgia,serif;'
        'font-size:1rem;font-weight:700;color:#fff;text-decoration:none;line-height:1.35;">'
        + a['title_es'] + '</a>'
        '</div>'
        '<div style="background:rgba(0,0,0,.15);padding:.5rem 1.1rem;">'
        '<span style="font-size:10px;color:rgba(255,255,255,.7);">'
        + flag(a['lang']) + ' ' + a['source'] + ' · ' + a['time'] + '</span>'
        '</div>'
        '</div>'
    )
...
def mini_card(a, color):
    return (
        '<div style="display:flex;background:#fff;border-radius:8px;border:1px solid #e8e2d8;'
        'overflow:hidden;margin-bottom:6px;">'
        '<div style="width:4px;background:' + color + ';flex-shrink:0;"></div>'
        '<div style="padding:.6rem .9rem;flex:1;">'
        '<a href="' + a['link'] + '" target="_blank" style="font-family:Georgia,serif;'
        'font-size:.9rem;font-weight:600;color:#1a1208;text-decoration:none;'
        'line-height:1.35;display:block;margin-bottom:3px;">' + a['title_es'] + '</a>'
        '<span style="font-size:10px;color:#a09688;">'
        + flag(a['lang']) + ' ' + a['source'] + ' · ' + a['time'] + '</span>'
        '</div>'
        '</div>'
    )
...
def sec_header(icon, label, color):
    return (
        ...
        'color:' + color + ';white-space:nowrap;">' + icon + ' ' + label + '</span>'
...
s_hist = (
    sec_header("📖","HISTORIA DEL DIA","#92400e") +
    '<div style="background:#fffbeb;border-radius:10px;border:1px solid #fde68a;'
    'padding:1.1rem 1.2rem;margin-bottom:8px;">'
    '<div style="font-size:2rem;text-align:center;margin-bottom:.6rem;">' + historia['emoji'] + '</div>'
    '<div style="font-family:Georgia,serif;font-size:.98rem;font-weight:700;color:#78350f;'
    'text-align:center;margin-bottom:.6rem;">' + historia['titulo'] + '</div>'
...
+ preheader + " · Al Dia Newsletter</div>\n"
...
"<p style='font-size:9px;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,.4);margin:0 0 .4rem;'>"
"Tu resumen diario · " + date_full + "</p>\n"
"<h1 style='font-family:Georgia,serif;font-size:2.2rem;font-weight:700;color:#f5e6c8;"
"letter-spacing:-1px;line-height:1;margin:0 0 .3rem;'>Al Dia</h1>\n"
"<p style='font-size:11px;color:rgba(255,255,255,.5);margin:0 0 .75rem;'>"
"Mantente informado con lo que importa hoy</p>\n"
"<span style='display:inline-block;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.15);"
"color:rgba(255,255,255,.75);font-size:10px;font-weight:600;padding:4px 14px;border-radius:20px;'>"
+ date_full + " &nbsp;·&nbsp; 3 min de lectura</span>\n"
"</div>\n"

# Greeting
"<div style='background:#fdf8f0;border:1px solid #e8e2d8;border-top:none;padding:1rem 1.4rem;'>\n"
"<p style='font-size:13.5px;color:#5c5248;line-height:1.65;margin:0;'>" + greeting + "</p>\n"
"</div>\n"
...
# Markets — data already embedded from Python
"<div style='background:#1a1208;padding:.9rem 1rem;'>\n"
"<div style='font-size:9px;letter-spacing:2px;text-transform:uppercase;"
"color:rgba(255,255,255,.4);text-align:center;margin-bottom:.6rem;'>"
"MERCADOS · " + time_display + " ET</div>\n"
"<div style='display:grid;grid-template-columns:repeat(3,1fr);gap:8px;'>\n"
+ markets_html +
"</div>\n"
"<div style='font-size:9px;color:rgba(255,255,255,.25);text-align:center;margin-top:.5rem;'>"
"Fuente: Yahoo Finance · CoinGecko · BCE</div>\n"
"</div>\n"
...
"Comparte Al Dia con tu familia y amigos 📲</p>\n"
"</div>\n"

# Footer
"<div style='background:#fff;border-radius:0 0 12px 12px;border:1px solid #e8e2d8;"
"border-top:none;padding:1rem 1.4rem;text-align:center;'>\n"
"<p style='font-size:12px;color:#5c5248;margin-bottom:4px;'>"
"Hecho con cafe · <strong style='color:#1a1208;'>Al Dia</strong> · Actualizado " + time_display + "</p>\n"
"<p style='font-size:10px;color:#a09688;margin-bottom:6px;'>"
"BBC · Reuters · The Guardian · El Pais · El Universo · DW Espanol</p>\n"
"<a href='https://leoya06.github.io/mi-briefing/' "
"style='font-size:11px;color:#2563eb;text-decoration:none;'>Ver Mi Briefing completo</a>\n"
"</div>\n"
